...
short int get_ldt_page()
{
	char* page_flags = PAGES_COUNT_FIELD_SIZE + (char*)memory;
	short int i = MEMORY_OFFSET;
	for (; i < PAGES_COUNT; ++i)
	{
		if (page_flags[i] == LDT_FLAG)
		{
			short int* in_page_ldt_count = (short int*)(i * PAGE_SIZE + (char*)memory);
			if (*in_page_ldt_count < PAGE_SIZE / LDT_SIZE)
				return i;
		}
	}

	short int ldt_page = get_next_free_page(0);
	short int* in_page_ldt_count = (short int*)(ldt_page * PAGE_SIZE + (char*)memory);
	*in_page_ldt_count = 0;
	set_page_flag(ldt_page, 1, LDT_FLAG);
	return ldt_page;
}

int get_new_ldt_address() 
{
	short int ldt_page = get_ldt_page();
	short int* in_page_ldt_count = (short int*)(ldt_page * PAGE_SIZE + (char*)memory);
	int in_page_ldt_offset = *in_page_ldt_count * LDT_SIZE + 2;
	int ldt_address = ldt_page * PAGE_SIZE + in_page_ldt_offset;
	*in_page_ldt_count += 1;
	return ldt_address;
}
...
int alloc_ldt(size_t pages)
{
	...
	*free_pages -= 1;
	...
	int ldt_address = get_new_ldt_address();
